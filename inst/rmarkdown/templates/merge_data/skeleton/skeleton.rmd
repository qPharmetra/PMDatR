...
for(dom in DMO$Domains) 
{
  ...
  # overwrite the draft
  #cat(xpd, file=output_name)
  #rmarkdown::render(output_name, quiet=T)
  
  #cat(sprintf("[%s Domain Load Report](%s_diagnostics.html)\n\n", dom$name, dom$name))
...
```{r}
EX.df = Process_Dose()
EX.df = post_process_dosing(EX.df, expandADDL = DMO$MergeOptions$ExpandADDL, 
                            ADDLTolerance = DMO$MergeOptions$ADDLTolerance)
kable(head(EX.df,nPrintRows))

kable(as_data_frame(unit_cols(EX.df)))
```
...
kable(as_data_frame(unit_cols(DV.df)))
```
...
for(df in Cov.l){
  ...
  print(kable(as_data_frame(unit_cols(df))))
}
```
...
kable(as_data_frame(unit_cols(CovT.df)))
```
...
kable(as_data_frame(unit_cols(database.df)))
```
